...
% TODO: add a sequence diagram (goal -> Nav2 -> /cmd_vel -> relay ->
% TCP -> bridge -> HAL -> motor) - this chapter describes the exact
% data flow in prose/tables, but a single figure would help the reader
% a lot given how many hops are involved.
...
\section{The ROS~2 Dashing / Humble Incompatibility}
\label{sec:rmw-incompatibility}

As described in Section~\ref{sec:qcar-platform}, the QCar's onboard
Jetson TX2 ships with ROS~2 Dashing (released 2019) pre-installed,
while this project's navigation stack targets ROS~2 Humble (released
2022). Section~\ref{sec:ros2-model} established that ROS~2 nodes
communicate via DDS participants that must share a compatible
discovery and wire protocol to find each other at all; the working
hypothesis at the start of this thesis's hardware phase was that this
would be a soft compatibility problem -- degraded performance,
perhaps, but not a hard failure.

This was tested directly, not assumed. A minimal ROS~2 publisher
(\texttt{demo\_nodes\_cpp talker}) on the QCar and a minimal
subscriber (\texttt{demo\_nodes\_py listener}) on the development PC,
both configured with the same \texttt{ROS\_DOMAIN\_ID}, never
established communication across three repeated attempts, with the
talker independently confirmed to be actively publishing each time.
The network layer itself was systematically ruled out before
concluding this was a middleware-level problem: raw UDP unicast
passed cleanly in both directions, including on the exact Fast~DDS
discovery port for the domain in use; no firewall was active on
either host; and an explicit unicast-peers Fast~DDS XML profile,
which bypasses multicast-based discovery entirely, still failed to
achieve discovery. This isolates the failure to the DDS protocol
...
ROS~2 Humble respectively \cite{eprosima_fastdds}, not a network,
firewall, or configuration problem that could be worked around within
ROS~2 itself.

This result rules out every architecture that assumes native ROS~2
interoperability between the two machines, including running a
Humble-compatible container on the QCar: a Humble Docker container
was considered early on, but does not address a protocol-version
incompatibility at the DDS transport layer, since the Dashing side of
the link is fixed by what physically runs on the vehicle's ROS~2
partition and cannot itself be upgraded without departing from the
...
into the motor socket, and publishes \texttt{sensor\_msgs/LaserScan}
...
plugin, requiring no awareness that the underlying vehicle is real
...
complete line is parsed as one JSON object, and a malformed line is
logged and discarded rather than allowed to crash the control loop --
deliberately fail-soft, since this loop is also driving the physical
motor and must keep running even if one message is corrupt.
...
failure mode rather than being an arbitrary convenience. First,
\textbf{TCP rather than UDP}: TCP guarantees in-order, complete
...
packets under loss with no delivery guarantee at all. For velocity
...
extra tens of milliseconds of latency (which the \texttt{CMD\_TIMEOUT}
watchdog in Section~\ref{sec:onboard-bridge-odom} already treats as a
safety condition regardless of its cause), so TCP's reliability
guarantee was preferred over UDP's lower-latency-but-lossy profile.
Second, \textbf{newline-delimited JSON rather than a fixed-length or
...
with a bare \texttt{nc}/\texttt{telnet} session during debugging (a
real, exercised advantage throughout this project's development, not
a theoretical one) and requires no shared binary schema to keep in
sync between the QCar-side Python and dev-PC-side Python. This is
also why \texttt{buf} accumulates across \texttt{recv()} calls rather
than assuming one \texttt{recv()} returns exactly one message: TCP's
byte-stream semantics make no guarantee that message boundaries align
with any particular \texttt{recv()} call's return, so a message can
legitimately arrive split across two calls, or two messages can
arrive concatenated in one call -- Listing~\ref{lst:bridge-recv-loop}'s
\texttt{while b'\textbackslash n' in buf} loop handles both cases
uniformly by always operating on whatever has accumulated so far.
Third, \textbf{one full-duplex socket carrying both directions}
rather than two separate one-way sockets for commands and odometry:
a single TCP connection is inherently bidirectional (each endpoint
can read and write independently on the same socket), so
Section~\ref{sec:onboard-bridge-odom}'s later addition of odometry
sent back to the relay required no new listening port or connection
-- only using a direction of the existing socket that had, until
then, been opened but left unread.

% TODO: insert a figure here illustrating the full path from a
% Nav2-issued velocity command to the physical motor (Nav2 ->
% /cmd_vel -> qcar_relay_node.py -> TCP -> qcar_bridge.py -> HAL ->
% motor). Give it \label{fig:bridge-architecture} and reference it
% with Figure~\ref{fig:bridge-architecture} in the sentence above once
% added.

Both onboard TCP servers were built to handle \texttt{SIGTERM}
identically to \texttt{SIGINT} -- a deliberate correctness fix, not a
default: Python does not automatically translate a plain
\texttt{pkill}'s default \texttt{SIGTERM} into a caught, clean-exit
signal the way it does for \texttt{SIGINT} (Ctrl-C), and an earlier
...
which is both a battery drain and a safety concern for anyone near
the vehicle. Section~\ref{sec:safe-shutdown} presents the resulting
...
itself the tell that a second error remained: it exactly mirrored the
original error about the vehicle's forward axis rather than
correcting it, which is the expected signature of fixing only a sign
error while a separate rotational offset error is still present. The
...
verified against a real obstacle at a known bearing (the vehicle's
physical right side), cross-checked against the rendered robot
model's own orientation in RViz rather than the screen's raw
left/right, which depends on the RViz camera's own facing direction
and is therefore not a reliable reference on its own.
...
removing it, because negating the angle alone corrects the sense of
increasing angle (clockwise vs.\ counter-clockwise) without also
correcting \emph{where zero itself points} -- two logically
independent properties of an angular convention, which is exactly why
this bug needed two independent corrections rather than one.
...
work end-to-end. Localisation (AMCL) does not share this property:
unlike Cartographer's self-contained scan matching, AMCL looks up a
live \texttt{odom} $\rightarrow$ \texttt{base} TF at every scan
callback as its motion-model input, so the wheel-odometry extension
originally scoped only for the mapping phase became a hard
...
Equations~\ref{eq:bicycle-x}--\ref{eq:bicycle-theta}, sent back to
the relay over the same already-full-duplex TCP connection the drive
commands arrive on (the reverse direction of this connection had
previously been opened but its data discarded).
...
approximately 45\,Hz, and a 3-second commanded straight-line drive at
\SI{0.3}{\meter\per\second} produced $x \approx \SI{1.25}{\meter}$,
$v \approx \SI{0.37}{\meter\per\second}$, $y \approx 0$ -- consistent
with straight-line motion at close to the commanded speed, and
confirmed working before the full navigation stack was brought up on
top of it.
...
rate follow, and $x$/$y$/yaw are then integrated forward exactly as
...
HAL manual for this drive motor/encoder pairing, and
\SI{0.01977}{\meter} is the linear distance one rear-wheel revolution
advances the vehicle (i.e.\ the wheel's rolling circumference,
folding in the fixed gear ratio between motor shaft and wheel), so
their ratio converts a raw encoder delta directly into a travelled
distance in one multiplication, with no separate gear-ratio or
...
$\cos(\text{yaw})$/$\sin(\text{yaw})$ -- effectively assuming the
...
interval. This is a standard, deliberate simplification (matching how
Chapter~\ref{ch:sim-stack}'s simulation-side plugin also integrates
its own motion incrementally rather than analytically per step), and
its error shrinks toward zero as the control loop's period
\texttt{dt} shrinks, but it is not exact for any nonzero
\texttt{yaw\_rate * dt} -- at this project's \SI{50}{\hertz} control
rate (Section~\ref{sec:bridge-architecture}), \texttt{dt} is small
enough that the resulting error is negligible next to this thesis's
other, much larger sources of odometry inaccuracy (steering trim,
throttle calibration), but it is worth naming explicitly as a
...
A small but consistent steering bias -- the physical vehicle drifting
laterally under a nominally straight (zero-steering) command -- was
present on this specific chassis, and the physical steering linkage
...
the value to roughly the right range, and a series of real
...
refined it further. This data carried real measurement noise --
lateral offset depends on the vehicle's initial aim at the start of
each test run, not purely on trim -- so the calibration deliberately
did not over-fit a precise value from only two data points; a
consistent directional nudge toward zero observed drift converged
faster in practice than attempting a precise interpolation. This
first-pass value was accepted once a \SI{4.4}{\meter} test drive
...
exact zero -- and, as the next section shows, it was in fact only half
of the real correction this vehicle needed.

\subsection{Steering Gain: A Second, Independent Nonlinearity}
...
correct amount of real wheel deflection, and for a long period of
...
the vehicle's operator reported that autonomous navigation on real
...
\paragraph{A false lead, and why it is reported here anyway.} The
first investigation session produced data that looked like a hard
nonlinearity: a sharp left/right asymmetry (the right side matching
the kinematic prediction, the left side showing roughly six times the
expected deflection), and, on a later resumption, an apparent narrow
``backlash snap'' between two adjacent commanded angles. Both were
artefacts, not steering-mechanism properties, and both were only
discovered by direct physical inspection rather than further data
analysis: a nut had become physically lodged under one wheel for an
unknown subset of test runs, obstructing motion outright on some
trials and adding partial drag on others that still nominally
completed; separately, the test battery had drifted to a
near-depleted \SI{11.9}{\volt}, and the \SI{0.3}{\meter\per\second}
test speed used that session sat at or below the drifted
static-friction floor characterised in
Section~\ref{sec:deadband-discovery} -- one ``successful'' run's own
logged cruise velocity showed only $\approx$\SI{0.22}{\meter\per\second}
actual speed for a \SI{0.3}{\meter\per\second} command, against a
validated \SI{0.31}{\meter\per\second} at the same command from a
fresh-battery session weeks earlier. Neither confound is a property of
the steering system, and every conclusion drawn from that dataset was
retracted rather than reconciled with the clean data below. This
episode is included in full, rather than quietly omitted in favour of
only the clean result, because it is the same methodological lesson
Section~\ref{sec:stray-process} draws from a different bug: an
apparently sharp, structured signal in real-hardware data is not, on
its own, evidence of a matching structured cause, and a physical
inspection of the test rig is sometimes a faster and more reliable
diagnostic step than further statistical analysis of already-collected
data.

\paragraph{A clean re-test.} With the nut removed, a fresh battery
fitted, and the test speed raised to \SI{0.4}{\meter\per\second} (confirmed
via logged velocity to sit safely above the stiction floor), a seven-point
sweep of commanded steering angles was driven, each a fresh
\SI{1}{\meter} arc from a consistent physical start pose, with
\texttt{STEERING\_TRIM} held at zero so the commanded value was exactly
what reached \texttt{car.write()}. The real wheel angle implied by
...
further). Two conclusions follow directly: first, the true zero-point
trim is $\delta_{\text{cmd}} = -0.09$, not the $-0.08$ found by the
coarser static test above, pinned by direct interpolation between the
$-0.085$ (implied $+0.005$) and $-0.09$ (implied $0.000$) rows; second,
and independently of the trim value entirely, the real wheel turns
approximately 39\% \emph{further} than the kinematic model's commanded
angle implies for any non-zero command -- a genuine multiplicative
gain error, not an offset, and one that a pure trim correction cannot
fix. This gain error is a strong candidate explanation for the
original complaint that motivated the investigation: Nav2's planner
and MPPI controller both assume the kinematic model of
Section~\ref{sec:true-ackermann} maps 1:1 onto real wheel angle, so a
...
hardware manual documents only the raw servo command range and the
physical maximum wheel angle (Table~\ref{tab:app-safety-limits}), and
the HAL's own \texttt{steeringBias} construction parameter was
confirmed left at its default of zero in \texttt{qcar\_bridge.py} --
nothing at the firmware level was fighting or duplicating the software
correction. \texttt{STEERING\_GAIN = 1.39} (Listing~\ref{lst:trim-split}'s
...
\texttt{STEERING\_TRIM = -0.087}. A dedicated validation drive --
commanding a real kinematic turn of \SI{0.15}{\radian} over a
\SI{1}{\meter} arc at \SI{0.4}{\meter\per\second}, predicting
\SI{28.5}{\centi\meter} of lateral deflection -- measured
\SI{27}{\centi\meter}, a residual error of $\approx$5.6\%, down from
$\approx$39\% with no gain correction at all. The fix was subsequently
confirmed in genuine closed-loop use: re-running autonomous Nav2
navigation on the physical vehicle after deployment, the operator's
direct assessment was that turning behaviour had visibly improved,
consistent with the gain error having been the dominant cause of the
original complaint rather than merely a contributing factor.
...
magnitude can produce more real curvature than expected, so available
...
Section~\ref{sec:onboard-bridge-odom}. Since the entire purpose of
...
Listing~\ref{lst:trim-split}. Note that \texttt{target\_steering} --
\texttt{compute\_steering()}'s untrimmed return value -- is what
...
principle this illustrates -- and which is stated here explicitly
because it is expected to generalise to any future per-unit hardware
calibration constant on this or a similar platform -- is that a
calibration constant compensating for a specific actuator's physical
imperfection should be applied \emph{as late as possible, immediately
at the actuator boundary}, and never upstream of any computation
...
The first real navigation-goal test run independently by this
project's author, after mapping, localisation, and the fixes above
were all in place, surfaced a new symptom: sudden AMCL localisation
jumps and visible path deviation on both straight and curved
segments. The description of what was actually observed --
LiDAR scan shape stays internally coherent, but the entire scan
rigidly shifts and appears to track the robot's motion, ``like the
skeleton of the map is moving'' -- together with an explicit
instinct that this might be a transform-related issue rather than a
tuning problem, pointed toward a \emph{timing} rather than a spatial
or gain-tuning cause, and this proved correct.
...
times are decorrelated, a late-arriving scan could be paired --
purely by receipt-time coincidence -- against an odometry sample
that, in real physical time, was already stale by a different,
independent amount. This produces exactly the observed symptom: a
...
A naive measurement -- record a local timestamp $t_1$, SSH into the
QCar and read its clock to get $t_{\text{remote}}$, then compute
$t_{\text{remote}} - t_1$ -- conflates two effects that a single
one-way measurement cannot separate: the genuine clock offset between
the two machines, and the real, nonzero time the SSH round trip
itself takes to establish a connection and execute the remote
command, which inflates the apparent gap in one direction only. The
correction used here follows the same symmetric-delay assumption
underlying NTP itself: record a second local timestamp $t_2$
immediately after the remote reply arrives, and treat the remote
reading as having been taken at the network's best available estimate
of the corresponding local instant, the midpoint $(t_1+t_2)/2$ -- this
assumes the outbound and return legs of the round trip take
approximately equal time, which is reasonable for a short, otherwise
idle local-network SSH round trip but would not be for a highly
asymmetric link. Averaging five such trials, rather than trusting a
single one, further reduces the influence of any one trial's
round-trip time deviating unusually from that symmetric-delay
assumption. This offset was
deliberately \emph{not} corrected via a system-level NTP/chrony
configuration, since the QCar is shared laboratory hardware, and a
persistent system configuration pointing it at one researcher's
personal development machine as a time reference is not appropriate
to leave on shared infrastructure. Instead, \texttt{qcar\_bridge.py}
and \texttt{qcar\_lidar\_node.py} were changed to stamp every message
with their own local \texttt{time.time()} at the moment of capture (a
\texttt{'t'} field in the JSON payload), and
\texttt{qcar\_relay\_node.py} gained a \texttt{qcar\_clock\_offset}
parameter and a \texttt{qcar\_time\_to\_stamp()} helper that converts
a QCar-local capture timestamp into its development-PC-clock
equivalent, applied consistently to both the \texttt{/odom}+TF and
\texttt{/scan} publication paths, shown in Listing~\ref{lst:clock-offset-code}.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{@{}ll@{}}
    ...
    Measured clock offset (round-trip corrected) & $\approx \SI{0.50}{\second}$ \\
    \texttt{qcar\_clock\_offset} launch parameter (verified session) & \texttt{0.50} \\
    \bottomrule
  \end{tabular}
\end{table}
...
motor bridge was still listening on its TCP port after the operator
believed it had been stopped -- so the documented procedure
