...
found, on real hardware, delivering zero messages in practice
...
reached in practice on physical hardware
...
\begin{description}
  ...
  \item[Global planning] Nav2's \texttt{SmacPlannerHybrid} plugin,
    ...
    \cite{dolgov2008hybridastar}, respecting the vehicle's minimum turning radius
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[
    ...
    \node[block] (resample) at (2.5,-2.6) {3. Resampling};

    \draw[arr] (predict) -- (update);
    \draw[arr] (update) -- (resample);
    \draw[arr] (resample.west) .. controls (-1.4,-1.6) and (-1.4,-0.6) .. (predict.south west);
  ...
  \end{tikzpicture}
  \caption[The AMCL predict-update-resample cycle]{The AMCL particle filter's prediction-update-resampling cycle, repeated on every new odometry/LiDAR pair: particles are propagated by the motion model, reweighted by how well each hypothesised pose explains the actual LiDAR scan, then resampled proportional to weight so the particle cloud statistically converges toward the true pose. The augmented-MCL recovery mechanism of Equation~\ref{eq:amcl-recovery} modifies step 3 only, occasionally drawing a fresh, uniformly random particle instead of resampling from the existing weighted set.}
...
Its onboard compute is an NVIDIA Jetson TX2 system-on-module,
...
The IMU is electrically present and exposed by the vendor's HAL/PAL
library (\texttt{gyroscope}/\texttt{accelerometer} fields), but
...
\begin{figure}[htbp]
  ...
  \caption[The QCar's front-mounted sensor stack]{The QCar's front-mounted sensor stack: the Intel RealSense
    D435 RGBD camera and the RPLiDAR unit sit above the carrier board,
    with the LiPo battery pack (labelled ``DO NOT PULL'') and
    Ackermann front suspension visible below. This thesis's navigation
    pipeline uses only the LiDAR, as discussed below.}
