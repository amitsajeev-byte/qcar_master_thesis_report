...
\pagenumbering{roman}
\input{frontmatter/titlepage}          % no page number, per guideline 4.1.1

\input{frontmatter/acknowledgement}    % guideline 4.1.3 - precedes Authorship per guideline order
\clearpage

% Declaration - mandatory per abschlussarbeit_anmeldung_master_mss_en.pdf,
% bound directly after title page, no page number. Embedded as the
% signed PDF page (frontmatter/signed_declaration.pdf) rather than
% \input{frontmatter/authorship} directly, so the bound copy carries
% the actual signature/date rather than blank fill-in lines.
...
% Same signed PDF, page 2.
\includepdf[pages=2,pagecommand={\thispagestyle{empty}}]{frontmatter/signed_declaration.pdf}

\input{frontmatter/abbreviations}       % per guideline 4.1.5 - precedes Abstract per guideline order
\clearpage

\input{frontmatter/abstract}           % 120-150 words, per guideline 4.1.6
\clearpage
...
\input{chapters/02_groundwork}
\input{chapters/03_simulation_stack}
\input{chapters/04_hardware_bridge}
\input{chapters/05_calibration}
\input{chapters/06_navigation_integration}
