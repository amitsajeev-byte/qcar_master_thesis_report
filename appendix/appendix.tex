...
presented in this thesis. The PWM and steering/overcurrent limits are
discussed in Section~\ref{sec:qcar-platform}
\cite{quanser2023qcar,quanser2023troubleshooting}. The battery
voltage thresholds are documented only in Quanser's troubleshooting
manual \cite{quanser2023troubleshooting} and are not otherwise
...
reproduced here in full. This follows the guideline's own
recommendation that programming code be supplied on the electronic
data carrier rather than printed.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{@{}p{3.8cm}p{9.5cm}@{}}
    ...
    \texttt{\seqsplit{IsPathValidDebouncedCondition}} & Custom BT.CPP condition node requiring consecutive failed validity checks before replanning; implemented and hardware-tested, currently reverted/deselected (Section~\ref{sec:ispathvalid-reverted}) \\
...
\section{QCar Bring-Up Command Reference}
\label{app:commands}
...
\begin{lstlisting}[language=bash,caption={QCar bring-up (two on-board terminals, two development-PC terminals).}]
...
thesis is supplied in the accompanying repository:
\url{https://github.com/amitsajeev-byte/qcar_nav_stack}.
